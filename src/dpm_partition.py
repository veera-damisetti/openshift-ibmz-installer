...
class DpmPartition():
    def __init__(self, name, disk_type, network_type,partition): 
        self.name = name
    ...
    def get_status(self):
        ...
        try:
            return self.partition.get_property('status')
        except zhmcclient.Error as e:
            logger.error("Error retrieving status for DPM partition %s: %s", self.name, str(e))
            return None
    
    def update_properties(self, ftp_host, ftp_username, ftp_password, ftp_insfile):
    ...
    def get_disk_ids(self,disk_type):
        try:
            logger.debug("Retrieving attached storage groups for partition %s.", self.name)
            storage_groups = self.partition.list_attached_storage_groups(full_properties=True)
        except zhmcclient.Error as e:
        ...
        disk_type = "fc" if disk_type.lower() == "dasd" else disk_type

        logger.debug("Looking for storage group of type %s for partition %s.", disk_type, self.name)
        for sg in storage_groups:
            if sg.get_property("type").lower() == disk_type.lower() and sg.get_property("shared") == False:
    ...
    def get_cpc(self):
        ...
        try:
            return self.partition.manager.parent.name
        except zhmcclient.Error as e:
            logger.error("Error retrieving CPC for partition %s: %s", self.name, str(e))
            return None
    
    def get_network_info(self, network_type):
    ...
    def get_mac_address(self, network_type, network_card):
    ...
    def get_partition_info(self):
        ...
        try:
            partition_info['cp_procs']= self.partition.get_property("cp-processors")
            partition_info['ifl_procs'] = self.partition.get_property("ifl-processors")
            partition_info['initial_memory'] = self.partition.get_property("initial-memory")
            partition_info['max_mem'] = self.partition.get_property("maximum-memory")
            partition_info['reserved_memory'] = self.partition.get_property("reserved-memory")
        except zhmcclient.Error as e:
